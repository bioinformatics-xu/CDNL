...
class PySigmoidFocalLoss(nn.Module):
    def __init__(self, gamma=2.0, alpha=0.5, reduction='mean'):
        super(PySigmoidFocalLoss, self).__init__()
        self.gamma = gamma
        self.alpha = alpha
    ...
    def forward(self, pred, target, weight=None, avg_factor=None):
        ...
        # target = F.one_hot(target, num_classes=pred.size(1)).type_as(pred)
    ...
    def __init__(
        ...
        *,
        ...
        if self.class_weight is not None:
            ...
            # )
            self.criterion = PySigmoidFocalLoss(
                weight=self.class_weight, reduction="mean"
            )
        if isinstance(self, LassoNetCoxRegressor):
            assert (
...
class LassoNetClassifier(
    ...
    criterion = PySigmoidFocalLoss(reduction="mean")
    ...
    def score(self, X, y, sample_weight=None):
        ...
        actual = reshape_data_into_2_dim(y)
        predicted = reshape_data_into_2_dim(self.predict_proba(X)[:, 1])
